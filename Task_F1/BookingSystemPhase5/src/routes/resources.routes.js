...
router.post("/", resourceValidators, async (req, res) => {
  ...
  try {
    ...
    // Add log event (success)
    const resourceId = rows[0].id;
    const successMsg = `RESOURCE_CREATED: name="${rows[0].name}", price=${Number(
      rows[0].price
    ).toFixed(2)}, unit="${rows[0].price_unit}", id=${resourceId}`;
    await logEvent({
      actorUserId,
      message: successMsg,
      entityType: "resource",
    ...
    if (err && err.code === "23505") {
      // Add log event (duplicate blocked)
      console.error(err);
      const dupMsg = `RESOURCE_CREATE_BLOCKED_DUPLICATE: name="${resourceName}", attempted_by_user_id=${actorUserId ?? "NULL"}`;
      await logEvent({
        actorUserId,
        message: dupMsg,
        entityType: "resource",
