...
const resourceDescContainer = document.getElementById("resourceDescContainer");  // New container for description
...
function createResourceDescInput(container) {
  ...
  // Core attributes
  input.id = "resourceDesc";
  input.name = "resourceDesc";
  input.rows = 5;
...
// Set input visual state based on validation
...
function attachResourceNameValidation(input) {
  const update = () => {
    ...
    setButtonEnabled(createButton, valid && isResourceDescValid(resourceDescInput.value));
  };
...
function attachResourceDescValidation(input) {
  const update = () => {
    ...
    setInputVisualState(input, valid ? "valid" : "invalid");
    setButtonEnabled(createButton, valid && isResourceNameValid(resourceNameInput.value));
  };
